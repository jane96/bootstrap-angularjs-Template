...
app.factory('utilsService', ['$sce', 'SweetAlert', function($sce, SweetAlert) {
	...
	factory.avatar = function(url, title, size) {
		...
		if (angular.isDefined(url) && url != '') {
			...
			var avatars = getColorCode(title);
			imgSrc = '<button class="btn btn-rounded btn-' + size + ' btn-icon btn-' + avatars.color + '">' + avatars.title + '</button>';
		}
	...
	//公用颜色封装，通过用户标题获取颜色标识
	factory.color = function(title) {
		return getColorCode(title).color;
	};

	function getColorCode(title) {
		if (title.length > 2) {
